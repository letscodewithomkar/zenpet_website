...
var typed = new Typed(".auto-type", {
    strings: ["Pet Grooming", "Pet Boarding", "Pet Training", "Pet Adoption", "Pet Massage", "Nutritional Counseling", "Pet Rehabilitation"],
    typeSpeed: 150,
    backSpeed: 150,
    loop: true
})
let planbuybtn = document.querySelectorAll('.planbuybtn')
let plansboneimgs = document.getElementById('plansboneimgs');

function subscriptionplan(value) {
    if (value == 'Bronze') {
        planbuybtn[0].setAttribute("src", "./webimg/zen pets bone subscription bronze.png");

    }
    else if (value == 'Gold') {
        planbuybtn[1].setAttribute("src", "./webimg/zen pets bone subscription gold.png");

    } else {
        planbuybtn[2].setAttribute("src", "./webimg/zen pets bone subscription platinum.png");
...
const getUsername = async () => {
    try {
        const response = await fetch("http://localhost/zenpets%20website/get_username.php");
...
const updateCart = async (cartlist) => {
    if (retrievedcart.length > 0) {
        ...
        fetch(`https://zenpet.onrender.com/update_cart.php`, {
            method: 'POST',
            headers: {
